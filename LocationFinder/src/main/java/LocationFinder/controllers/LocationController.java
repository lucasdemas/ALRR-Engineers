...
public class LocationController {
     ...
     *
     * @param e The MissingServletRequestParameter Exception
    ...
    public ResponseEntity<?> handleInvalidNumber(
        ...
        return new ResponseEntity<>("The Authentication token"
     ...
     * @param clientAuthToken The authentication token of
     *                        the client attempting to add a new location
    ...
    public ResponseEntity<?> addNewLoc(@RequestParam final
        ...
        try {
            ...
            //verify that there is a client with
            //the provided client id in the database
            ...
            //make sure that the client id the auth token is associated to
            //match the one passed in the request
            //if they match proceed to check the other inputs to
            //see if they are all valid and add the new location
            ...
            if (clientId.equals(authClient.getId())) {
                ...
                Location loc = new Location(locName,
                        locArea, locCost, clientId);

                //Check that all the data the user input is in a valid format
                ...
                System.out.printf("The client with authentication token %s"
                                + " has successfully added a new location "
                                + "with the values: locName: %s,"
                                + " locArea: %s, locCost: %f, clientId: %d%n",
                        decryptedToken, locName, locArea, locCost, clientId);
                return new ResponseEntity<>(savedLocation, HttpStatus.CREATED);
            } else {
                System.out.printf("A client has attempted to add a new "
                                + "location to"
                                + " the client with the id %d using the "
                                + "token: %s which is not the token"
                                + "associated with that client.%n",
                        clientId, decryptedToken);
                return new ResponseEntity<>("The auth token does not "
                        + "match the client id "
                        + "that you are attempting to create a location for!",
     ...
     *
     * @param clientId        The id of the client whose locations
     *                        are being retrieved
     * @param clientAuthToken The authentication token provided to
     *                        retrieve all the client's locations
    ...
    public ResponseEntity<?> getLocations(@RequestParam final
        ...
        try {
            ...
            //make sure that the client id the auth token is associated to
            //match the one passed in the request
            //if they match proceed to retrieve a list of locations that
            //belong to the specified client id
            //if they do not match, throw an error
            if (clientId.equals(authClient.getId())) {
                //Return all the locations with the specified client id
                Iterable<Location> ownedLocs =
                        locRepository.getAllByClientId(clientId);
                System.out.printf("The client with id %d and "
                                + "authentication token %s"
                                + " has successfully retrieved all their "
                                + "location from the database.%n",
                         clientId, decryptedToken);
                return new ResponseEntity<>(ownedLocs, HttpStatus.OK);
            } else {
                System.out.printf("A client has attempted to retrieve "
                        + "all the locations of the client"
                        + " with the id %d using the "
                        + "token: %s which is not the token associated with "
                        + "that client.%n", clientId, decryptedToken);
                return new ResponseEntity<>("The auth token does not match "
                        + "the client id "
                        + "that you are attempting retrieve the location of!",
                        HttpStatus.FORBIDDEN);
            }
     ...
     * @param clientId        The id of the client who's filtering
     *                        their locations by a specific area
     * @param clientAuthToken The authentication token provided to
    ...
    public ResponseEntity<?> getLocByArea(@RequestParam final
        ...
        try {
            ...
            //match the one passed in the request
            //if they match proceed to retrieve a list of locations that
            //belongs to the specified client id
            //if they do not match, throw an error
            if (clientId.equals(authClient.getId())) {
                //Return all the locations with the specified client id
                List<Location> areaLocs =
                        locRepository.findByArea(area, clientId);
                System.out.printf("The client with authentication token %s"
                        + " has successfully retrieved all their locations "
                        + "from the area: %s"
                        + " from the database.%n",
                        decryptedToken, area);
                return new ResponseEntity<>(areaLocs, HttpStatus.OK);
            } else {
                System.out.printf("A client has attempted to retrieve all "
                                + "the locations of the client"
                                + " in the area %s, client with the id %d "
                                + "using the "
                                + "token: %s which is not the token associated"
                                + " with that client.%n",
                        clientId, area, decryptedToken);
                return new ResponseEntity<>("The auth token does not "
                        + "match the client id "
                        + "that you are attempting retrieve the location of!",
                        HttpStatus.FORBIDDEN);
    ...
    public ResponseEntity<?> getLocByClaim(@RequestParam final
        ...
        try {
            ...
            //verify that there is a client
            //with the provided client id in the database
            clientServ.getClientById(clientId);
            ...
            //make sure that the client id the auth token is associated
            //to match the one passed in the request
            //if they match proceed to retrieve a list of locations
            //that belong to the specified client id
            //if they do not match, throw an error
            if (clientId.equals(authClient.getId())) {
                //Get a list of the locations that are either claimed
                //or unclaimed specified by the input
                List<Location> searchResults =
                        locService.getLocationByClaim(isClaim, clientId);
                System.out.printf("The client with client id %d "
                                + "and authentication token %s"
                                + " has successfully retrieved "
                                + "all their %s locations"
                                + " from the database.%n",
                        clientId, decryptedToken, isClaim);
                return new ResponseEntity<>(searchResults, HttpStatus.OK);
            } else {
                System.out.printf("A client has attempted to retrieve all "
                                + "the %s locations of the client"
                                + " with the id %d using the "
                                + "token: %s which is not the token "
                                + "associated with that client.%n",
                        isClaim, clientId, decryptedToken);
                return new ResponseEntity<>("The auth token does not"
                        + " match the client id "
                        + "that you are attempting retrieve the location of!",
                        HttpStatus.FORBIDDEN);
            ...
            return new ResponseEntity<>(e.getMessage(),
                 ...
                 | NoSuchPaddingException | NotFoundException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
        } catch (IOException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.FORBIDDEN);
        }
    ...
    public ResponseEntity<?> updateLocCost(@RequestParam final
        ...
        try {
            ...
            //if they do not match, throw an error
            if (clientId.equals(authClient.getId())) {
                //Check to see if the location is in the DB and get its data
                Location targetLoc = locService.getLocById(locId);

                //check to make sure the client id provided matches the client
                //id that the location belongs to
                if (clientId.equals(targetLoc.getClientId())) {
                    ...
                    Location updatedLoc =
                            locService.updateLocCost(targetLoc, locCost);
                    System.out.printf("The client with authentication token %s"
                                    + " has successfully updated "
                                    + "the location with"
                                    + " the location id: %d to the new cost"
                                    + " of %f in the database.%n",
                            decryptedToken, locId, locCost);
                    return new ResponseEntity<>(updatedLoc, HttpStatus.OK);
                } else {
                    System.out.printf("A client with the client id: %d "
                                    + "has attempted to update the cost "
                                    + "of the location with the "
                                    + " location id %d to the cost of %f "
                                    + "which does not belong to them.%n",
                            clientId, locId, locCost);
                    return new ResponseEntity<>("The location who's cost you "
                ...
                System.out.printf("A client has attempted to to update the "
                                + "cost of the location with the"
                                + " location id %d to the cost of %f belong "
                                + "to the client with the "
                                + "client id %d with the authentication "
                                + "token: %s which is not the token associated"
                                + " with that client.%n",
                        locId, locCost, clientId, decryptedToken);
                return new ResponseEntity<>("The auth token does not match "
                        + "the client id that you are attempting to update the"
                        + " cost of a location!", HttpStatus.FORBIDDEN);
            }
        } catch (NotFoundException | NoSuchAlgorithmException
                | NoSuchPaddingException e) {
            return new ResponseEntity<>(e.getMessage(), HttpStatus.NOT_FOUND);
     ...
     * @param clientId        The id of the client associated with
     *                        the location.
     *                        For authentication usage.
     * @param clientAuthToken The authentication token
     *                        provided to verify the post request
    ...
    public ResponseEntity<?> updateLocClaim(@RequestParam final
        ...
        try {
            ...
            //to match the one passed in the request
            //if they match proceed to retrieve a list of locations
            //that belongs to the specified client id
            //if they do not match, throw an error
            if (clientId.equals(authClient.getId())) {
                //Check to see if the location is in the DB and get its data
                Location targetLoc = locService.getLocById(locId);

                //check to make sure the client id provided matches
                // the client id that the location belongs to
                if (clientId.equals(targetLoc.getClientId())) {
                    //Update the location's data in the
                    //database given the provided information by the user
                    Location updatedLoc =
                    ...
                    if (locClaim) {
                        System.out.printf("The client with authentication"
                                        + " token %s has successfully updated"
                                        + " the claimed status of the "
                                        + "location with the location id: %d"
                                        + " to claimed in the database.%n",
                                decryptedToken, locId);
                    } else {
                        System.out.printf("The client with authentication "
                                        + "token %s has successfully"
                                        + " updated the claimed status of the "
                                        + "location with the location id: %d"
                                        + " to unclaimed in the database.%n",
                                decryptedToken, locId);
                    }
                    return new ResponseEntity<>(updatedLoc, HttpStatus.OK);
                } else {
                    if (locClaim) {
                        System.out.printf("A client with the client id: %d "
                                        + "has attempted to update the claim "
                                        + "status of the location with the "
                                        + " location id %d to claimed "
                                        + "but the location"
                                        + " does not belong to them.%n",
                                clientId, locId);
                    } else {
                        System.out.printf("A client with the client id: %d "
                                        + "has attempted to update the "
                                        + "claim status of the location "
                                        + "with the location id %d to "
                                        + "unclaimed but the location"
                                        + " does not belong to them.%n",
                                clientId, locId);
                    }
                    return new ResponseEntity<>("The location who's cost you "
                            + "are trying to update does not belong to the "
                            + "client information provided!",
                            HttpStatus.FORBIDDEN);
                }
            } else {
                if (locClaim) {
                    System.out.printf("A client has attempted to to update "
                                    + "claim status of the location with the"
                                    + " location id %d to claimed belonging "
                                    + "to the client with the "
                                    + "client id %d with the authentication  "
                                    + "token: %s which is not the token "
                                    + "associated with that client.%n",
                            locId, clientId, decryptedToken);
                } else {
                    System.out.printf("A client has attempted to to update "
                                    + "claim status of the location with the"
                                    + " location id %d to unclaimed belonging "
                                    + "to the client with the client id %d "
                ...
                return new ResponseEntity<>("The auth token does not "
                        + "match the client id that you are attempting to "
                        + "update the cost of a location!",
                        HttpStatus.FORBIDDEN);
            }
        } catch (NotFoundException | NoSuchAlgorithmException
                | NoSuchPaddingException e) {
     ...
     *      The authentication token provided to the entrypoint to
     *      perform the action
    ...
    public ResponseEntity<?> deleteLoc(@RequestParam final
        ...
        try {
            ...
            //verify that there is a client with the provided
            //client id in the database
            clientServ.getClientById(clientId);
            ...
            if (clientId.equals(authClient.getId())) {
                ...
                if (clientId.equals(targetLoc.getClientId())) {
                    ...
                    System.out.printf("The client with authentication"
                            + " token %s has successfully deleted the "
                            + "location with the location id: %d"
                            + " in the database.%n", decryptedToken, locId);
                    return new ResponseEntity<>("Location deleted"
                            + " successfully!", HttpStatus.OK);
                } else {
                    System.out.printf("A client with the client id: %d "
                                    + "has attempted to delete the location "
                                    + "with the location id %d but the"
                                    + " location does not belong to them.%n",
                            clientId, locId);
                    return new ResponseEntity<>("You cannot delete a location"
                            + " that does not belong to you!",
                            HttpStatus.FORBIDDEN);
                }
            } else {
                System.out.printf("A client has attempted to delete the "
                                + "location with the"
                        ...
                        locId, clientId, decryptedToken);
                return new ResponseEntity<>("You cannot delete a location "
                        + "that you do not have the credentials for!",
                        HttpStatus.FORBIDDEN);
            }
        } catch (NotFoundException | NoSuchAlgorithmException
                | NoSuchPaddingException e) {
